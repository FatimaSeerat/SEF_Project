...
import { ArrowDown, Brain, Calendar } from "lucide-react";

const HeroSection = () => {
  return (
    <section
      id="register-section"
      className="relative min-h-screen flex items-center justify-center bg-gradient-hero noise"
    >
      {/* Grid Background */}
      <div className="absolute inset-0 bg-grid-pattern opacity-1000000" />
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10 pt-20">
        <div className="flex flex-col items-center text-center max-w-5xl mx-auto">
          ...
          {/* Subheading */}
          <p className="text-sm md:text-[18px] max-w-3xl mb-8 mt-2 animate-fade-in-up opacity-0 delay-200">
          ...
          {/* CTA Buttons */}
          <div className="flex flex-col sm:flex-row items-center gap-4 mb-12 animate-fade-in-up opacity-0 delay-400 w-full sm:w-auto justify-center">
                ...
                <div className="relative bg-card rounded-xl p-6 md:p-8">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <Link to="/quiz" className="block w-full">
                      <div className="bg-muted/50 rounded-xl p-5 border border-border/50 hover:border-primary/30 transition-all group cursor-pointer h-full">
                        ...
                        <div className="space-y-2">
                          <div className="h-3 bg-muted rounded w-full flex items-center px-2">
                            <span className="text-[10px] text-foreground/70 font-medium">What is photosynthesis?</span>
                          </div>
                          <div className="h-3 bg-muted rounded w-4/5 flex items-center px-2">
                            <span className="text-[10px] text-foreground/70 font-medium">Define Newton's Laws</span>
                          </div>
                          <div className="h-3 bg-muted rounded w-3/5 flex items-center px-2">
                            <span className="text-[10px] text-foreground/70 font-medium">Explain mitosis</span>
                          </div>
                        </div>
                    ...
                    <Link to="/timetable" className="block w-full">
                      <div className="bg-muted/50 rounded-xl p-5 border border-border/50 hover:border-secondary/30 transition-all group cursor-pointer h-full">
                        <div className="flex items-center gap-3 mb-4">
                        ...
                        <div className="grid grid-cols-5 gap-1.5">
                          {Array.from({ length: 15 }).map((_, i) => {
                            const labels = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Math', '', 'Bio', '', '', 'Phys', '', 'Chem', '', ''];
                            return (
                              <div
                                key={i}
                                className={`h-5 rounded-md transition-all flex items-center justify-center ${[2, 5, 7, 11, 13].includes(i)
                              ...
                              >
                                <span className="text-[8px] text-foreground/70 font-medium">{labels[i]}</span>
                              </div>
                            );
                          })}
                        </div>
